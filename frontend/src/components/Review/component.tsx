...
export const Review: FunctionComponent<{ review: FeedBack }> = ({ review }) => {
    ...
    <div className={styles.review}>
      ...
      <div className={styles.review__block}>
        <div className={styles.review__top}>
          <h3 className={styles.review__user}>{name}</h3>
          <div className={styles.review__rating}>
            <span>Оценка: </span>
            <span className={styles.review__count}>{rating}</span>
          </div>
        </div>
        <p className={styles.review__descr}>{text}</p>
      </div>
    </div>
  );
};
